...
static std::vector<std::string> roles = {"Admin", "Receptionist", "KitchenStaff"};
static int activeField = 0; // 1-6 for add fields, 0=none

static void reload() { employees = getAllEmployees(); branches = getAllBranches(); }
static void clearInp() {
    for (int i = 0; i < 7; i++) memset(inputs[i], 0, 100);
    roleIdx = 0; branchIdx = 0;
    roleDropOpen = false; branchDropOpen = false;
    activeField = 0;
}
...
void InitStaffPanel() {
    reload();
    showAddPopup = showEditPopup = showDeactivateDialog = false;
    clearInp();
    editUsername = deactUser = "";
    scrollOffset = 0.0f;
...
void DrawStaffPanel() {
    ...
    if (DrawButton({px + pw - 160, py - 5, 150, 36}, "+ Add New Staff", COL_ACCENT, COL_WHITE)) {
        clearInp();
        showAddPopup = true; showEditPopup = false; showDeactivateDialog = false;
    }
    ...
    for (size_t i = 0; i < employees.size(); i++) {
        ...
        if (DrawButton({ax, y + 2, 55, 28}, "Edit", COL_IN_PREP, COL_WHITE)) {
            ...
            snprintf(inputs[0], 100, "%s", e.firstName.c_str());
            snprintf(inputs[1], 100, "%s", e.lastName.c_str());
            snprintf(inputs[2], 100, "%s", e.email.c_str());
            snprintf(inputs[3], 100, "%s", e.phone.c_str());
        ...
        if (DrawButton({ax + 62, y + 2, 85, 28}, "Deactivate", COL_LOW_STOCK, COL_WHITE)) {
            deactUser = e.username;
            showDeactivateDialog = true; showAddPopup = false; showEditPopup = false;
        }
    }
    ...
    if (showDeactivateDialog) {
        bool sd = true;
        if (DrawConfirmDialog("Deactivate Staff", "Deactivate " + deactUser + "?", sd)) {
            bool ok = deactivateEmployee(deactUser);
            setStatus(ok ? "Staff deactivated" : "Error", ok);
            reload();
        }
        showDeactivateDialog = sd;
    }

    // ==================== ADD POPUP ====================
    if (showAddPopup) {
        DrawPopupOverlay();
        float pw2 = 500, ph2 = 500, px2 = (WINDOW_WIDTH - pw2) / 2, py2 = (WINDOW_HEIGHT - ph2) / 2;
        ...
        for (int f = 0; f < 6; f++) {
            DrawText(labels[f], (int)ix, (int)iy, FONT_SIZE_SMALL, COL_DARK);
            Rectangle fieldRect = {ix + 150, iy - 5, pw2 - 210, 36};
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && CheckCollisionPointRec(mouse, fieldRect))
                activeField = f + 1;
            DrawInput(fieldRect, "", inputs[f], 99, activeField == f + 1, (f == 5)); // Password masked
            iy += 48;
        }

        DrawText("Role:", (int)ix, (int)iy, FONT_SIZE_SMALL, COL_DARK);
        DrawDropdown({ix + 150, iy - 5, pw2 - 210, 36}, "", roles, roleIdx, roleDropOpen);
        iy += 48;

        DrawText("Branch:", (int)ix, (int)iy, FONT_SIZE_SMALL, COL_DARK);
        std::vector<std::string> bn; for (auto& b : branches) bn.push_back(b.branchName);
        ...
        if (DrawButton({px2 + 30, btnY, 120, 36}, "Save", COL_ACCENT, COL_WHITE) || IsKeyPressed(KEY_ENTER)) {
            ...
            if (strlen(inputs[0]) > 0 && strlen(inputs[1]) > 0) {
                bool ok = addEmployee(inputs[0], "", bid, inputs[1], inputs[2], inputs[3], inputs[4], inputs[5], roles[roleIdx]);
                setStatus(ok ? "Staff added" : "Error", ok);
                reload();
                showAddPopup = false; clearInp();
            }
        }
    ...
    if (showEditPopup) {
        DrawPopupOverlay();
        float pw2 = 500, ph2 = 420, px2 = (WINDOW_WIDTH - pw2) / 2, py2 = (WINDOW_HEIGHT - ph2) / 2;
        ...
        for (int f = 0; f < 4; f++) {
            ...
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && CheckCollisionPointRec(mouse, fieldRect))
                activeField = f + 1;
            DrawInput(fieldRect, "", inputs[f], 99, activeField == f + 1, false);
            iy += 48;
        }
        ...
        if (IsKeyPressed(KEY_ESCAPE)) { showEditPopup = false; clearInp(); }

        // Buttons
        float btnY = py2 + ph2 - 55;
        if (DrawButton({px2 + 30, btnY, 120, 36}, "Update", COL_ACCENT, COL_WHITE) || IsKeyPressed(KEY_ENTER)) {
            int bid = (branchIdx < (int)branches.size()) ? branches[branchIdx].branchId : 1;
            bool ok = updateEmployee(editUsername, inputs[0], inputs[1], inputs[2], inputs[3], roles[roleIdx], bid);
            setStatus(ok ? "Staff updated" : "Error", ok);
            reload();
            showEditPopup = false; clearInp();
        }
        if (DrawButton({px2 + pw2 - 150, btnY, 120, 36}, "Cancel", COL_BORDER, COL_DARK)) {
            showEditPopup = false; clearInp();
        }
