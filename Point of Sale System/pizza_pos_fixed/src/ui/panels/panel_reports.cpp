...
// Inline report structs (mirror db_reports.cpp)
...
void InitReportsPanel() {
    ...
    generated = false; branchDropOpen = false;
}
...
void DrawReportsPanel() {
    ...
    std::vector<std::string> brNames = {"All Branches"}; for (auto& b : branches) brNames.push_back(b.branchName);
    DrawDropdown({px, py + 35, 220, 36}, "Branch:", brNames, branchIdx, branchDropOpen);
    DrawText("From:", (int)px + 240, (int)py + 40, FONT_SIZE_SMALL, COL_DARK); DrawInput({px + 290, py + 35, 110, 32}, "", dateFrom, 19, true, false);
    DrawText("To:", (int)px + 420, (int)py + 40, FONT_SIZE_SMALL, COL_DARK); DrawInput({px + 450, py + 35, 110, 32}, "", dateTo, 19, true, false);
    if (DrawButton({px + 580, py + 35, 140, 32}, "Generate Report", COL_ACCENT, COL_WHITE)) {
        ...
        for (auto& o : orders) {
            ...
            for (auto& oi : getOrderItems(o.orderId)) {
                ...
                bool f = false; for (auto& p : counts) { if (p.first == nm) { p.second += oi.quantity; f = true; break; } } if (!f) counts.push_back({nm, oi.quantity});
            }
        }
        salesData.netRevenue = salesData.totalRevenue - salesData.totalDiscounts;
        for (size_t i = 0; i < counts.size(); i++) for (size_t j = i + 1; j < counts.size(); j++) if (counts[j].second > counts[i].second) std::swap(counts[i], counts[j]);
        salesData.topItems = counts;

        invData.clear(); auto inv = getInventory(bid > 0 ? bid : getBranchId());
        for (auto& i : inv) invData.push_back({i.inventoryName, i.unit, i.quantity, i.reorderLevel, i.isLowStock});

        staffData.clear(); auto emps = getAllEmployees(bid);
        for (auto& e : emps) {
    ...
    if (!generated) { DrawText("Select filters and click Generate Report", (int)px, (int)(ty + 50), FONT_SIZE_NORMAL, COL_BORDER); return; }

    float cy = ty + 45;
    if (activeTab == 0) {
        float cw2 = (pw - 40) / 4.0f;
        auto sc = [&](Rectangle r, const char* l, const char* v, Color c) { DrawCard(r, "", COL_CARD); DrawText(l, (int)r.x + 10, (int)r.y + 10, FONT_SIZE_SMALL, COL_BORDER); float tw = (float)MeasureText(v, FONT_SIZE_TITLE); DrawText(v, (int)(r.x + (r.width - tw) / 2), (int)(r.y + 30), FONT_SIZE_TITLE, c); };
        char b1[32], b2[32], b3[32], b4[32];
        snprintf(b1, sizeof(b1), "%d", salesData.totalOrders); snprintf(b2, sizeof(b2), "%.0f", salesData.totalRevenue);
        snprintf(b3, sizeof(b3), "%.0f", salesData.totalDiscounts); snprintf(b4, sizeof(b4), "%.0f", salesData.netRevenue);
        ...
        for (size_t i = 0; i < salesData.topItems.size() && i < 8; i++) {
            float bw = (maxQ > 0) ? ((pw - 200) * salesData.topItems[i].second / maxQ) : 10; if (bw < 10) bw = 10;
